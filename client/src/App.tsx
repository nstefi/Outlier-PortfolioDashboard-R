...
const DashboardContent = () => {
  ...
  const calculateTotalShares = (): number => {
    return stocks.reduce((total, stock) => {
      return total + (stock.shares || 0);
    }, 0);
  };
  
  const calculateTotalDailyChange = (): number => {
    return stocks.reduce((total, stock) => {
      const shares = stock.shares || 0;
      return total + (stock.change * shares);
    }, 0);
  };
  
  const calculateTotalDailyChangePercent = (): number => {
    const portfolioValue = calculateTotalPortfolioValue();
    if (portfolioValue === 0) return 0;
    
    const totalChange = calculateTotalDailyChange();
